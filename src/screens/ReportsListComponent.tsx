...
const ReportsList = observer( ({route,navigation}:any) => {
    ...
    const [filtredReport, setFiltredReport] = useState<ReportDto[]>(reports);
    const addReport=()=>{
      setShowPicker(true);
    }
  ...
  const filtreReportByDate=(date:Date)=>{
    const filtred = reports.slice().filter(r=>r.date===format(date,SIMPLE_DATE_FORMAT));
    setFiltredReport(filtred);
  }
  const isValid=(item:ReportDto)=>{
    console.log('----- is valid -------', item.status)
